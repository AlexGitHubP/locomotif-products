...
<div class='filter-tab'>
	...
	<div class='actions-tab'>
		<ul class='action-controls'>
			<li>
				<a href="/admin/products/create" class='iconControl'>
					<span>
	...
	<div class='listing-elements-hold'>
		@foreach($products as $key => $product)
			<div class='listing-element {{ $loop->last ? 'lastElement' : '' }}'>
				...
				<div class='listing-box flex03 alignCenter'>
					<p>{{\Carbon\Carbon::parse($product->created_at)->format('Y-d-m')}}</p>
				</div>
				<div class='listing-box flex03 alignCenter'>
					<p>{{$product->price}} RON</p>
				</div>
				<div class='listing-box flex04 alignCenter'>
					<span class='general-btn noPointer status-{{$product->product_status}}'>{{$product->product_status_nice}}</span>
				</div>
				<div class='listing-box flex02 alignCenter'>
					<div class='more-actions-tab'>
						...
						<ul class='more-list'>
							<li>
								<form action="/admin/products/{{ $product->id }}" method="POST">@method('DELETE') @csrf <input type="submit" value="Șterge"/></form>
							</li>
						</ul>
					</div>
				</div>
